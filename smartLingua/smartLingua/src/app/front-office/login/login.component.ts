...
@Component({
  ...
  template: `
    <div class="auth-page">
      <div class="auth-card">
        <h1>Se connecter</h1>
        <p class="subtitle">Email et mot de passe.</p>
        <form (ngSubmit)="onSubmit()">
          <label>Email</label>
          <input type="email" [(ngModel)]="email" name="email" placeholder="ex. toi@email.com" required />
          <label>Mot de passe</label>
          <input type="password" [(ngModel)]="password" name="password" placeholder="••••••••" required />
          @if (errorMessage) {
            <p class="error">{{ errorMessage }}</p>
          }
          <button type="submit" class="btn-primary" [disabled]="loading">Se connecter</button>
        </form>
        <p class="link">Pas encore de compte ? <a routerLink="/register">Créer un compte</a></p>
...
export class LoginComponent {
  ...
  onSubmit() {
    ...
    if (!em || !pwd) {
      this.errorMessage = 'Renseigne email et mot de passe.';
      return;
    }
    this.loading = true;
    this.authApi.login(em, pwd).subscribe((res) => {
      this.loading = false;
      if (res.ok) {
        this.authApi.setSession(res.user);
        this.router.navigate(['/chat']);
      } else {
        this.errorMessage = res.message || 'Email ou mot de passe incorrect.';
      }
    });
  }
}
