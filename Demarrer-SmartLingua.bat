...
start "Backend - Messaging" cmd /k "cd /d "%~dp0backend\backend\backEnd\microservices\messaging" && mvnw.cmd spring-boot:run "-Dspring-boot.run.profiles=dev""

echo [2/2] Attente du demarrage du backend (~30 secondes)...
timeout /t 30 /nobreak >nul

echo Ouverture du FRONTEND (port 4200)...
start "Frontend - SmartLingua" cmd /k "cd /d "%~dp0smartLingua\smartLingua" && npm start"
