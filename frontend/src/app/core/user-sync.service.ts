...
export class UserSyncService {
  ...
  private async buildSyncDto(): Promise<UserSyncDto | null> {
    try {
      ...
      return {
        ...
        role: this.resolvePrimaryRoleFromToken(kc.tokenParsed as { realm_access?: { roles?: string[] } } | undefined)
      };
    } catch {
      return null;
    }
  ...
  private resolvePrimaryRoleFromToken(tokenParsed?: { realm_access?: { roles?: string[] } }): 'STUDENT' | 'TEACHER' | 'ADMIN' {
    const roles = (tokenParsed?.realm_access?.roles ?? []).map((r) => r.toUpperCase().replace(/^ROLE_/, ''));
