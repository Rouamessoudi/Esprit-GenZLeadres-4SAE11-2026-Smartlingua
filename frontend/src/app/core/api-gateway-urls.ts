...
export function aiAssistantGatewayPrefix(): string {
  if (isNgServe()) {
    return `${window.location.origin}/api-proxy/ai`;
  }
  return 'http://localhost:8095';
}

export function messagingGatewayPrefix(): string {
  if (isNgServe()) {
    return `${window.location.origin}/api-proxy/messaging`;
  }
  return 'http://localhost:8092';
}

export function forumGatewayPrefix(): string {
  if (isNgServe()) {
    return `${window.location.origin}/api-proxy/forum`;
  }
  return 'http://localhost:8096';
}

export function usersGatewayPrefix(): string {
  if (isNgServe()) {
    return `${window.location.origin}/api-proxy/users`;
  }
  return 'http://localhost:8093/users';
}
