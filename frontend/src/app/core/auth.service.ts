...
export class AuthService {
    ...
    register(): void {
        // Inscription publique: uniquement pour les apprenants (student) côté IAM/backend.
        // Teacher/Admin doivent être créés/validés par un administrateur.
        this.keycloakService.login({
    ...
    async logout(redirectPath: string = '/'): Promise<void> {
        ...
        try {
            await this.keycloakService.getKeycloakInstance().logout({ redirectUri });
        } catch (error) {
            console.error('Logout Keycloak failed, applying local fallback', error);
